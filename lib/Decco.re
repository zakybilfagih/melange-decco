...
let intFromJson = j =>
  ...
  | Some(f) =>
    Js.Float.isFinite(f) && Js.Math.floor_float(f) == f
      ? Belt.Result.Ok(Obj.magic(f): int)
...
let arrayFromJson = (decoder, json) =>
      ...
      ~f=
        (acc, jsonI, i) => {
          ...
          | (_, Belt.Result.Error({path, _} as error)) =>
            Belt.Result.Error({
              ...error,
              path: "[" ++ string_of_int(i) ++ "]" ++ path,
            })

          | (Belt.Result.Ok(prev), Belt.Result.Ok(newVal)) =>
            Belt.Result.Ok(Js.Array.concat(~other=[|newVal|], prev))
          }
        },
      ~init=Belt.Result.Ok([||]),
      arr,
  ...
  | None =>
    Belt.Result.Error({path: "", message: "Not an array", value: json})
  };
...
let resultFromJson = (okDecoder, errorDecoder, json) =>
  ...
  | Some([|variantConstructorId, payload|]) =>
    ...
    | Some("Ok") =>
      okDecoder(payload)->Belt.Result.map(v => Belt.Result.Ok(v))
